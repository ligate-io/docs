...
| `--faucet-url`   | `LIGATE_FAUCET_URL`       | Default `https://faucet.ligate.io`                       |
...
<Warning>
  **Broken in current releases (≤ v0.1.2-devnet).** The CLI's
  hardcoded default `https://faucet.ligate.io` no longer resolves
  (the standalone `ligate-io/faucet` service was archived; the live
  faucet is on `api.ligate.io/v1/drip`). The CLI also hardcodes
  `/faucet` as the path suffix, so `--faucet-url` can't paper over
  the host change either. Tracking:
  [ligate-cli#34](https://github.com/ligate-io/ligate-cli/issues/34).
  Until the fix ships, use `curl` directly (below).
</Warning>

Claim a one-shot drip from the public devnet faucet. Positional
address argument, no signing involved.

**Working alternative** (canonical endpoint, served by
[`ligate-api`](https://github.com/ligate-io/ligate-api)):
...
**Once the CLI is fixed**, the shape will be:
...
Override the faucet base URL with `--faucet-url` or
`LIGATE_FAUCET_URL`. Current default `https://faucet.ligate.io`
(dead); fix in flight.
...
ligate query attestation lsc1...:lph1...
# Compound id is `<schemaId>:<payloadHash>`
```
