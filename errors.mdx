...
**Fix for case 1:** drip via the working HTTP endpoint then wait
one block:
...
The CLI's `ligate faucet` is currently broken (defaults to a dead
hostname). See [`ligate faucet`](/cli#ligate-faucet) for the
full state.
